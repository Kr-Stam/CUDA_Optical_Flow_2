...
//mozhe e void*, ama dava type safety takada neka sedi
...
)
{
	...
	for (int i = 0; i < h; i += offset)
	{
		for (int j = 0; j < w; j += offset)
		{
			...
			cv::arrowedLine(
				test,
				cv::Point(j, i), cv::Point(tmp_nj, tmp_ni),
				cv::Scalar(0, 0, 255), 1, 8, 0, 0.4
			);
		}
	}
...
int main()
{
    ...
    while (true)
    {
        ...
        gpu::bilinear_filter(gray.data, gray.data, filtered.data, w, h, ww, wh, 2, 10);
